...
var SPECIAL_WEAPON_TIME = 1000 * 20;
var SCORE = {
  ASTEROID_DAMAGE: 10,
  ASTEROID_DESTROY: 50,
  UFO_DAMAGE: 0,
...
function init() {
  ...
  dom.weapon = document.getElementById('weapon');
...
function loop() {
  ...
  if (isPlay) {
    ...
    if (item) {
      ...
      if (item.vanished) {
        ...
        ship.setSpecialWepon(item.weapon);
        item = null;
      }
    }
    ...
    dom.weapon.innerHTML = ship.currentWepon.name;
  ...
  if (item) {
    context.beginPath();
    context.strokeStyle = item.weapon.color;
    context.lineWidth = 1;
    item.draw(context);
    context.stroke();
  }
...
function Ship(x, y, size) {
  Point.call(this, x, y);
  this.size = size;
  this.currentWepon = WEAPON_DEFAULT;
...
Ship.prototype = extend(new Point(), {
  ...
  died: false,
  splinter: null,
  setSpecialWepon: function(weapon) {
    this.specialWeponSetTime = new Date().getTime();
    this.currentWepon = weapon;
  },
  destroy: function() {
    this.died = true;
  ...
  update: function(mouse) {
    ...
    if (new Date().getTime() - this.specialWeponSetTime > SPECIAL_WEAPON_TIME) {
      this.currentWepon = WEAPON_DEFAULT;
    }
  },
  draw: function(ctx) {
...
function Item(x, y) {
  ...
  this.weapon = WEAPON_SPECIAL[randInt(WEAPON_SPECIAL.length - 1)];
